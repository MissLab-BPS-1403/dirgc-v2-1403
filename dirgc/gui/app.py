...
from dirgc.tambah_usaha_cli import run_tambah_usaha
...
def build_footer_label():
    footer = CaptionLabel(
        "Made with ❤️and ☕ - ATM dari Novanni Indi Pradana - OLEH IPDS 1400"
    )
...
@dataclass
class RunConfig:
    mode: str  # "dirgc" or "tambah_usaha"
    headless: bool
    manual_only: bool
...
class RunWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            if self._config.mode == "tambah_usaha":
                run_tambah_usaha(
...
class RunPage(QWidget):
    def __init__(self, sso_page=None, parent=None):
        ...
        title = TitleLabel("Automasi Matchapro")
        ...
        left_layout.addWidget(self._build_mode_card())
    ...
    def _build_mode_card(self):
        ...
        card_layout.addWidget(SubtitleLabel("Mode"))
        ...
        mode_layout.addWidget(StrongBodyLabel("Pilih mode proses"))
        self.mode_combo = ComboBox()
        self.mode_combo.addItems(["DIRGC (Update GC)", "Tambah Usaha Baru"])
        ...
        self.mode_desc = CaptionLabel(
            "Mode DIRGC: Filter usaha yang sudah ada lalu isi hasil GC."
        )
        self.mode_desc.setWordWrap(True)
        self.mode_desc.setStyleSheet("color: #6B6B6B;")
        card_layout.addWidget(self.mode_desc)
    ...
    def _on_mode_changed(self, index):
        if index == 0:  # DIRGC
            ...
            default_placeholder = "data/Direktori_SBR_20260114.xlsx"
        else:  # Tambah Usaha
            self.mode_desc.setText(
                "Mode Tambah Usaha: Tambahkan usaha baru tanpa filter/matching."
            )
            default_placeholder = "data/tambah_usaha.xlsx"
        
        self.excel_input.setPlaceholderText(default_placeholder)
        self._save_settings()
    ...
    def _load_settings(self):
        ...
        if "mode" in options:
            mode_index = 0 if options["mode"] == "dirgc" else 1
    ...
    def _save_settings(self):
        ...
        mode = "dirgc" if self.mode_combo.currentIndex() == 0 else "tambah_usaha"
        data["options"] = {
            "mode": mode,
    ...
    def _start_run(self):
        ...
        self._save_settings()
        mode_name = "DIRGC" if config.mode == "dirgc" else "Tambah Usaha"
        self.status_label.setText(f"Status: running ({mode_name})")
        self._set_running_state(True)
        self.progress_label.setText("Progress: memuat data...")
        self._append_log(f"=== START RUN: {mode_name.upper()} ===")
    ...
    def _confirm_start(self):
        mode_name = "DIRGC" if self.mode_combo.currentIndex() == 0 else "Tambah Usaha"
        if self._confirm_dialog(
            "Mulai proses",
            f"Mulai proses {mode_name} sekarang?",
        ):
            self._start_run()
...
class HomePage(QWidget):
    def __init__(self, parent=None):
        ...
        title = LargeTitleLabel("Matchapro Automation")
        subtitle = BodyLabel(
            "Alat bantu untuk mempercepat proses input hasil Ground Check "
            "dan Tambah Usaha Baru berdasarkan data Excel."
        )
        ...
        summary_text = BodyLabel(
            "Aplikasi mendukung 2 mode: (1) DIRGC untuk update hasil GC usaha yang sudah ada, "
            "(2) Tambah Usaha untuk menambahkan usaha baru tanpa filter/matching."
        )
        ...
        steps = [
            (
                "1. Pilih Mode",
                "DIRGC untuk update GC, atau Tambah Usaha untuk usaha baru.",
            ),
            (
                "2. Siapkan Excel",
            ...
            (
                "3. Isi Akun SSO",
                "Buka menu Akun SSO, aktifkan switch, lalu isi username dan password.",
            ),
            (
                "4. Jalankan",
...
class MainWindow(FluentWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Matchapro Automation")
