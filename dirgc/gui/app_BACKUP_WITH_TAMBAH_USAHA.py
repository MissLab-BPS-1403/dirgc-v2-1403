...
class RunConfig:
    ...
    web_timeout_s: int
    mode: str = "dirgc"  # "dirgc" atau "tambah_usaha"
...
class RunPage(QWidget):
    def __init__(self, sso_page=None, mode="dirgc", parent=None):
        super().__init__(parent)
        self._worker = None
        self._sso_page = sso_page
        self._recent_excels = []
        self._mode = mode  # "dirgc" atau "tambah_usaha"

        layout = QVBoxLayout(self)
        layout.setContentsMargins(24, 24, 24, 24)
        layout.setSpacing(16)

        # Title & subtitle berbeda untuk mode tambah usaha
        if self._mode == "tambah_usaha":
            title = TitleLabel("tambah usaha baru")
            subtitle = BodyLabel("Mode khusus untuk tambah usaha baru tanpa filter/matching.")
        else:
    ...
    def _build_files_card(self):
        ...
        # Placeholder berbeda untuk mode tambah usaha
        if self._mode == "tambah_usaha":
            self.excel_input.setPlaceholderText("tambah_usaha.xlsx")
        else:
    ...
    def _apply_default_paths(self):
        if self._mode == "tambah_usaha":
            excel_path = self._resolve_default_path("tambah_usaha.xlsx")
        else:
    ...
    def _get_settings_key(self, key):
        """Generate key berbeda untuk mode tambah_usaha"""
        if self._mode == "tambah_usaha":
            return f"tambah_usaha_{key}"
        return key

    def _load_settings(self):
        data = load_gui_settings()
        options_key = self._get_settings_key("options")
        excel_key = self._get_settings_key("excel_path")
        recents_key = self._get_settings_key("recent_excels")
        
        options = data.get(options_key, {})
        excel_path = data.get(excel_key)
        recents = data.get(recents_key, [])
    ...
    def _save_settings(self):
        ...
        data[excel_key] = self.excel_input.text().strip()
        data[recents_key] = self._recent_excels
        data[options_key] = {
    ...
    def _confirm_start(self):
        if self._mode == "tambah_usaha":
            title = "Mulai Tambah Usaha"
            message = "Mulai proses tambah usaha baru sekarang?"
        else:
            title = "Mulai proses"
            message = "Mulai proses sekarang?"
        
        if self._confirm_dialog(title, message):
            self._start_run()
...
class MainWindow(FluentWindow):
    def __init__(self):
        ...
        self.run_page = RunPage(self.sso_page, mode="dirgc", parent=self)
        self.tambah_usaha_page = RunPage(self.sso_page, mode="tambah_usaha", parent=self)
        self.settings_page = PlaceholderPage("Settings", self)
        
        self.home_page.setObjectName("home_page")
        self.run_page.setObjectName("run_page")
        self.tambah_usaha_page.setObjectName("tambah_usaha_page")
        ...
        self.addSubInterface(self.tambah_usaha_page, FIF.ADD, "Tambah Usaha")
    ...
    def closeEvent(self, event):
        ...
        if self.tambah_usaha_page:
            self.tambah_usaha_page._save_settings()
