...
def add_usaha(
    ...
    """
    Fungsi untuk menambahkan usaha baru ke DIRGC
    Menggunakan Select2 dropdown untuk wilayah
    """
    ...
    if tambah_button.count() == 0:
        tambah_button = page.locator("button.btn-tambah-usaha")
    if tambah_button.count() == 0:
    ...
    # Helper untuk isi input text
    def safe_fill_input(field_id, value, field_name):
        if not value:
            return
        
        locator = page.locator(f"#{field_id}")
        if locator.count() == 0:
            log_warn(f"Field {field_name} tidak ditemukan.")
            return
        ...
        try:
            ...
            log_error(f"Failed to fill {field_name}.", error=str(exc))
    
    # Helper untuk Select2 dropdown
    def safe_select2(container_id, search_value, field_name):
        if not search_value:
            return
        
        try:
            # Klik container untuk buka dropdown
            container = page.locator(f"#{container_id}")
            if container.count() == 0:
                log_warn(f"{field_name} container tidak ditemukan.")
                return
            ...
            if search_field.count() == 0:
                log_warn(f"{field_name} search field tidak ditemukan.")
                page.keyboard.press("Escape")
                return
            
            search_field.first.fill(search_value)
            monitor.wait_for_condition(lambda: False, timeout_s=1)
            
            # Pilih hasil pertama
            result = page.locator(".select2-results__option").first
            if result.count() > 0:
                monitor.bot_click(result)
                log_info(f"Selected {field_name}.", value=search_value)
            else:
                log_warn(f"No results for {field_name}.", value=search_value)
                page.keyboard.press("Escape")
        
        except Exception as exc:
            log_error(f"Failed to select {field_name}.", error=str(exc))
            try:
                page.keyboard.press("Escape")
            except:
                pass
    ...
    # Isi form
    safe_fill_input("tu-nama-usaha", nama_usaha, "Nama Usaha")
    safe_fill_input("tu-alamat-usaha", alamat, "Alamat")
    
    # Select2 untuk wilayah
    if provinsi:
        safe_select2("container-provinsi-tt", provinsi, "Provinsi")
        monitor.wait_for_condition(lambda: False, timeout_s=2)
    ...
    safe_fill_input("tu-latitude", latitude, "Latitude")
    safe_fill_input("tu-longitude", longitude, "Longitude")
    
    # Cari tombol Simpan
    simpan_button = page.locator("#modal-tambah-usaha button", has_text="Simpan")
    if simpan_button.count() == 0:
        simpan_button = page.locator("button.btn-primary", has_text="Simpan")
    ...
    try:
        simpan_button.first.scroll_into_view_if_needed()
    except Exception:
        pass
    
    log_info("Clicking Simpan button.")
    
    try:
        monitor.bot_click(simpan_button.first)
    except Exception as exc:
        log_warn("Failed to click Simpan.", error=str(exc))
        return False
    
    # Tunggu response
    success_keywords = ["berhasil", "sukses", "success"]
    duplicate_keywords = ["duplikat", "duplicate", "sudah ada"]
    ...
    def find_response():
        ...
        try:
            popup_text = popup.first.inner_text().lower()
        except:
            return False
        
        if any(keyword in popup_text for keyword in success_keywords):
            swal_result = "success"
            return True
        
        if any(keyword in popup_text for keyword in duplicate_keywords):
            swal_result = "duplicate"
            return True
        
        if "gagal" in popup_text or "error" in popup_text:
            swal_result = "error"
            return True
        
        return False
    
    monitor.wait_for_condition(find_response, timeout_s=30)
    
    if swal_result == "success":
        log_info("Usaha berhasil ditambahkan.")
        ok_button = page.locator(".swal2-confirm")
        ...
        log_warn("Usaha duplikasi terdeteksi.")
        ok_button = page.locator(".swal2-confirm")
        if ok_button.count() > 0:
            try:
                monitor.bot_click(ok_button.first)
        ...
        log_warn("Submit gagal atau timeout.")
        try:
            close_btn = page.locator("#modal-tambah-usaha .close")
